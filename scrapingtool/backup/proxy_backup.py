...
class Proxy():
    ...
    def __init__(self, proxy_port=9050, control_port=9051, OS='Windows', use_tor=True, country='in', stream_isolation=False, server_url='https://www.amazon.in', use_proxy=True):
        ...
        if self.stream_isolation == False:
            self.proxies = {
                'http': f'127.0.0.1:{self.proxy_port}',
                'https': f'127.0.0.1:{self.proxy_port}',
            }
        else:
    ...
    def switch_proxy(self):
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Switch to proxy {ip}")
        else:
            logger.warning(f"Proxy switching didn't work")        
    ...
    # Checks for listing and product url accessilbity through the new proxy
...
if __name__ == '__main__':
    proxy = Proxy(proxy_port=9050, control_port=9051, use_tor=False, use_proxy=True)
